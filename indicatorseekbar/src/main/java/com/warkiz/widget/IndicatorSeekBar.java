...
public class IndicatorSeekBar extends View {
    ...
    private int mType;//the num of tickMarks
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mType = ta.getInt(R.styleable.IndicatorSeekBar_isb_type, builder.type);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resolveSize(SizeUtils.dp2px(mContext, 170), widthMeasureSpec), height + mTickTextsHeight + 10);
        initSeekBarInfo();
        refreshSeekBarLocation();
    }

    private void initSeekBarInfo() {
        mMeasuredWidth = getMeasuredWidth() - 12;
...
//                mTextCenterX[i] = mPaddingLeft + mSeekBlockLength * i;
                if (i == 0)
                    mTextCenterX[i] = 24 + mSeekBlockLength * i;
                else if (i == mTickMarksX.length - 1)
                    mTextCenterX[i] = (mPaddingLeft + 24) + mSeekBlockLength * i;
                else
                    mTextCenterX[i] = mPaddingLeft + mSeekBlockLength * i;
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            if (i == 0) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] + mTickTextsWidth[index] / 2.0f, mTickTextY + 30, mTextPaint);
            } else if (i == mTickTextsArr.length - 1) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] - mTickTextsWidth[index] / 2.0f, mTickTextY + 30, mTextPaint);
            } else {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i], mTickTextY + 30, mTextPaint);
            }
        }
    }
    ...
    private void drawThumbText(Canvas canvas) {
        ...
        canvas.drawText(getProgressString(mProgress), getThumbCenterX(), mThumbTextY + 10, mTextPaint);
//        canvas.drawText(getProgressString(mProgress), getThumbCenterX(), 20, mTextPaint);
    }
