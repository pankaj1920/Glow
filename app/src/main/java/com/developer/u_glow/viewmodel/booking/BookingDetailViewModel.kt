...
class BookingDetailViewModel : BaseViewModel<BookingDetailState>() {

    private var offerList: MutableList<OfferData>? = null
    private var offerAdapter: OfferAdapter? = null

    private var state: BookingDetailState = BookingDetailState.Init
        set(value) {
            field = value
            publishState(state)
        }
    ...
    private fun setOfferAdapter() {
        ...
        state = BookingDetailState.SetOfferAdapter(offerAdapter)
    }
}